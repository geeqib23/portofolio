...
function Blog({ posts }) {
  ...
  return (
    <div className='container w-full h-full mx-auto my-5'>
      <p className='text-lg sm:text-xl'>I am a tech-geek and read a lot online. In my blogs and tweets, I will try to share with you the insights I have gained from my experiences. </p>
      <br></br>
      <div className='grid grid-cols-1 sm:grid-cols-2'>
      {posts.map( (post, index) => {
        return (
        <div key={index} onClick={() => router.push(`/blogs/${post.slug}`)} className='m-2 transition-all duration-200 transform rounded-lg cursor-pointer card bg-darkShade border-opacity-80 shadow-md hover:border-opacity-0 hover:-translate-y-2'>
          <div className='m-3'>
            <h2 className='mb-2 text-lg font-semibold text-blue-50'>{post.title}</h2>
            <p className='font-mono text-sm font-light transition-all duration-200 text-secondaryText '>{post.date}</p>
